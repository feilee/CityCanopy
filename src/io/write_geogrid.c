...
// unused definition. Conflicts with system definition
// #define BIG_ENDIAN    0
      ...
      int * wordsize )         /* number of bytes to use for each array element */
{
   ...
   */
   ...
   switch(*wordsize) {
      case 1:
         for(i=0; i<narray; i++) {
            // unsigned int is always positive => if condition is false
            // if (iarray[i] < 0 && *isigned) iarray[i] += (1 << 8);
            if (*isigned) iarray[i] += (1 << 8);
